...
export default defineEventHandler(async (event) => {
  ...
  if (!user.resetPasswordToken || !user.resetPasswordTokenValidUntil)
  ...
  if (user.resetPasswordTokenValidUntil < new Date()) throw createError({ statusCode: 401, message: "Bad credentials" })
  ...
  await useDrizzle()
    .update(users)
    .set({ password: hashedPassword, resetPasswordToken: null, resetPasswordTokenValidUntil: null })
