...
async function backgroundProcessEmail({ email }: { email: string }) {
  ...
  const resetLink = `https://app.klarbook.com/reset-password?token=${passwordResetToken}`
  const expiresAt = new Date(Date.now() + 1000 * 60 * 60 * 24)

  // Set the reset password token and expiration date
  await useDrizzle()
    .update(users)
    .set({ resetPasswordToken: passwordResetToken, resetPasswordTokenValidUntil: expiresAt })
    .where(eq(users.id, user.id))

  try {
    const emailResult = await postmark.sendEmail({
      from: "Klarbook Support <support@klarbook.com>",
      to: user.email,
      subject: "Passwort zurücksetzen",
      htmlBody:
        `<p>Hallo ${user.name},</p>` +
        `<p>du hast eine Anfrage zum Zurücksetzen deines Passworts für Klarbook erhalten. Um dein Passwort zurückzusetzen, klicke bitte auf den folgenden Link:</p>` +
        ...
        `<p>Vielen Dank<br />Dein Klarbook Team</p>`,
    })
